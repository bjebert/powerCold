...
get_predictions <- function(train_1, test_1, train_2, test_2, params = NULL) {
    ...
    mdl <- lm("consumption ~ mean_hour + mean_day_hour + I(mean_hour * mean_day_hour) + 0", test_1)
    predictions <- predict(mdl, test_2)
    
    mdl2 <- lm("consumption ~ mean_hour + 0", test_1)
    ind <- is.na(predictions)
    predictions[ind] <- predict(mdl2, test_2[ind])
