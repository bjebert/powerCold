...
preprocess_data <- function(adv_data) {
    ...
    x <- colnames(adv_data)
    x <- x[!x %in% c("series_id", "timestamp", "is_test", "series_N", "day", "hour")]
    
    adv_data <- adv_data[, x, with = FALSE]
...
get_predictions <- function(adv_train, adv_test, get_model = FALSE) {
    y_train <- adv_train[["is_test"]]
    
    adv_train <- preprocess_data(adv_train)
    adv_test <- preprocess_data(adv_test)
    
    Dtrain <- xgb.DMatrix(data = as.matrix(adv_train), label = y_train)
    Dtest <- xgb.DMatrix(data = as.matrix(adv_test))        
...
# 
#     nll <- -Metrics::logLoss(val2[["is_test"]], preds)
#     list(Score = nll,
#          Pred = preds)
# }
# 
